...
def train_model(
    ...
    # Log model artifacts to MLflow run
    ...
    # Model Registry Level 1: Explicitly register model to centralized MLflow Model Registry
    logger.info(f"Registering model to MLflow Model Registry: {fully_qualified_model_name}")
    registered_model = mlflow.register_model(
        ...
        tags={
            ...
            "rmse": str(rmse),
            "baseline_rmse": str(baseline_rmse),
        },
    )

    # Model Registry Level 2: Deployment governance and metric-threshold promotion gate
    client = mlflow.MlflowClient()
    if rmse < baseline_rmse:
        logger.info(f"Model beat baseline ({rmse:.1f} < {baseline_rmse:.1f}). Promoting to @{model_alias}")
        ...
        logger.warning(f"Model did not beat baseline ({rmse:.1f} >= {baseline_rmse:.1f}). Assigning @challenger alias for review.")
...
def main():
    ...
    try:
        ...
        logger.info(f"Reading features from Feature Store using FeatureEngineeringClient: {args.catalog}.{args.schema}.{args.feature_store_table_name}")
        dataframe = fe.read_table(f"{args.catalog}.{args.schema}.{args.feature_store_table_name}")
    except Exception:
        logger.info("Reading preprocessed data from feature store table.")
