...
def test_unpack_explodes_one_row_per_prediction(spark):
    from lego_ml_product.monitoring.unpack_inference_table import unpack_payload
    ...
    assert rows["req-1-0"]["Predicted_number_of_parts"] == pytest.approx(7.5)
    assert rows["req-1-1"]["Predicted_number_of_parts"] == pytest.approx(6.0)
    assert rows["req-2-0"]["Predicted_number_of_parts"] == pytest.approx(8.1)
...
def test_unpack_output_schema(spark):
    ...
    assert set(out.columns) == {
        "record_id",
        "prediction_ts",
        "Predicted_number_of_parts",
        "features",
        "model_version",
    }
    dtypes = dict(out.dtypes)
    assert dtypes["prediction_ts"] == "timestamp"
    assert dtypes["Predicted_number_of_parts"] == "double"
...
def test_empty_unpacked_table_returns_zero_rows_with_correct_schema(spark):
    ...
    """
    from lego_ml_product.monitoring.unpack_inference_table import empty_unpacked_table
